...
<%
	...
	int _pageRow = goodsBeanHandler.getGoodsBeanCategoryCount(category);
	int _pageCount = _pageRow % _pageSize == 0 ? _pageRow / _pageSize : _pageRow / _pageSize + 1;
	if (__page != null) {
		_page = Integer.parseInt(__page);
	}
	ArrayList<GoodsBean> arr = goodsBeanHandler.getGoodsBeanCategoryByPage(_page, _pageSize, category);
%>
...
<body>
	...
	<div id="content">
		<div id="content-left">
			<div id="books">
				<div class="title">
					<a href="index.jsp">主页</a>>><%=category.substring(1,category.length()-1) %>
				</div>
				<div class="gray">
					<img src="images/bullet1.gif"><%=category.substring(1,category.length()-1) %>
				</div>
				<%
				if(arr.size()<=0){
					out.println("<div style='font-size:15px;margin:10px 0 3px 1px;color:red;text-align:center;'>无该类书籍!</div>");
				}else{
